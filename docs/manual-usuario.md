...
**Modelo predictivo:** dos métodos disponibles, ambos deterministas (nunca calculados ni elegidos por una IA). Por defecto, **tasa de crecimiento promedio** entre los periodos históricos disponibles, aplicada al último valor conocido, con un rango de ± 1 desviación estándar de esa tasa (se ensancha si la tendencia ha sido volátil). Cuando hay **8 o más periodos** con datos válidos, aparece un botón "📈 Ver con regresión lineal" — un método que usa todos los puntos históricos a la vez, con un intervalo de predicción formal basado en el error estándar de la regresión. El usuario elige cuál ver, con un clic, las veces que quiera. Ninguno es un modelo de machine learning entrenado ni series de tiempo complejas (ARIMA) — decisión deliberada para que sean explicables y verificables por cualquier persona. Ver `docs/data_dictionary.md` para el detalle matemático completo de ambos métodos y por qué no se automatizó la elección entre ellos.
